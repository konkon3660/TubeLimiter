...
test('non-wrapping window is active within its start/end minutes on an included day', () => {
  const windows = [{ id: '1', label: '낮잠 방지', days: ALL_DAYS, startMinute: 13 * 60, endMinute: 14 * 60, enabled: true }];
...
test('wrapping window (22:00-07:00) is active both before and after midnight', () => {
  const windows = [{ id: 'w', label: '밤 시간', days: ALL_DAYS, startMinute: 22 * 60, endMinute: 7 * 60, enabled: true }];
...
test('a wrapping window belongs to the day it starts on, not the day it ends on', () => {
  const windows = [{ id: 'w', label: '월요일 밤', days: MON_ONLY, startMinute: 22 * 60, endMinute: 7 * 60, enabled: true }];
...
test('boundary: active exactly at startMinute, inactive exactly at endMinute', () => {
  const nonWrap = [{ id: 'w', label: 'x', days: ALL_DAYS, startMinute: 13 * 60, endMinute: 14 * 60, enabled: true }];
  ...
  const wrap = [{ id: 'w', label: 'x', days: ALL_DAYS, startMinute: 22 * 60, endMinute: 7 * 60, enabled: true }];
...
test('a disabled window is always ignored regardless of time', () => {
  const windows = [{ id: 'w', label: 'x', days: ALL_DAYS, startMinute: 0, endMinute: 1439, enabled: false }];
...
test('schedule evaluation uses the real wall-clock day, not the 4am usage-day cutoff', () => {
  ...
  const windows = [{ id: 'w', label: 'x', days: MON_ONLY, startMinute: 0, endMinute: 180, enabled: true }];
...
test('minutesUntilNextScheduleStart counts minutes to a start later today', () => {
  const windows = [{ id: 'w', label: 'x', days: ALL_DAYS, startMinute: 22 * 60, endMinute: 7 * 60, enabled: true }];
...
test('minutesUntilNextScheduleStart rolls over to the next matching day when today has passed', () => {
  const windows = [{ id: 'w', label: 'x', days: MON_ONLY, startMinute: 22 * 60, endMinute: 7 * 60, enabled: true }];
...
test('minutesUntilNextScheduleStart wraps to next week when the only matching day already passed today', () => {
  const windows = [{ id: 'w', label: 'x', days: WED_ONLY, startMinute: 8 * 60, endMinute: 9 * 60, enabled: true }];
...
test('minutesUntilNextScheduleStart ignores disabled windows', () => {
  const windows = [{ id: 'w', label: 'x', days: ALL_DAYS, startMinute: 13 * 60, endMinute: 14 * 60, enabled: false }];
